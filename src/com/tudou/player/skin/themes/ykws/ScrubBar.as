	...
	public class ScrubBar extends Widget
	{
		...
		private function dispatch(_time:Number):void
		{
			dispatchEvent( new NetStatusEvent
				...
				, false
				, false
				, { code:NetStatusCommandCode.SEEK, level:NetStatusEventLevel.COMMAND, data:{ id:this.id, time:_time }}
				)
			);
		...
		private function tweenToMin():void
		{
			...
			loadedTween.easeOut().from( { y:loaded.y, height:loaded.height } ).to( { y:3, height:3 }, 300);
			playedTween.easeOut().from( { y:played.y, height:played.height } ).to( { y:3, height:3 }, 300);
		...
		private function tweenToMax():void
		{
			...
			loadedTween.from( { y:loaded.y, height:loaded.height } ).to( { y:0, height:6 }, 200);
